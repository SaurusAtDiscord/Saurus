  ...
  async execute(Interaction, Args) {
    ...
    return Interaction.createFollowup(
      new Embed({
        ...
        fields: [
          {
            name: "Guild Details",
            value: `• Nickname: ${
              user.nick ?? "No Nickname"
            }\n• Joined at: ${moment(new Date(user.joinedAt)).format(
              "LL"
          ...
          {
            name: "User Details",
            value: `• Identity: ${user.username}#${user.discriminator} (\`${
              user.id
            }\`)\n• Registered: ${moment(new Date(user.createdAt)).format(
              "LL"
        ...
        color: this.client.extensions.string.splitNumbers(
          Interaction.channel?.guild.roles.get(roles[0])
        )?.color,
      }).parse()
    );
  }
