...
module.exports = class ButtonHelper {
  ...
  _addRowIfRequired() {
    if (
      this.rows.length === 0 ||
      this.rows[this.rows.length - 1].components.length >= this.maxRows ||
      this.rows[this.rows.length - 1].components[0]?.type === 3
    ) {
      this._createRow();
    }
  }
  ...
  _removeEmptyRows() {
    this.rows
      ...
      .forEach((row) => this.rows.splice(this.rows.indexOf(row), 1));
  }
