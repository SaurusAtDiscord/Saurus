...
module.exports = class Library extends SuperAgent {
  ...
  deleteCommand(commandId) {
    if (process.env.NODE_ENV === "development") {
      this.client.deleteGuildCommand("887396392497184778", commandId);
    } else this.client.deleteCommand(commandId);
  }

  createCommand(context) {
    if (process.env.NODE_ENV === "development") {
      this.client.bulkEditGuildCommands("887396392497184778", context);
  ...
  async getGuild(guildId) {
    return (
      ...
      (
        await this.get(`https://discord.com/api/v9/guilds/${guildId}`).set(
          "Authorization",
          `Bot ${process.env.KEY}`
        )
      ).body
  ...
  async getUser(userId) {
    return (
      ...
      (
        await this.get(`https://discord.com/api/v9/users/${userId}`).set(
          "Authorization",
          `Bot ${process.env.KEY}`
        )
      ).body
  ...
  async getMember(userId, guildId) {
    return (
      ...
      ).set("Authorization", `Bot ${process.env.KEY}`)
    ).body;
  }
};
