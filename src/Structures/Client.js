...
module.exports = class SaurusNode extends Client {
  ...
  loadCommands(dir) {
    readdirSync(dir).forEach((f) => {
      const directory = readdirSync(`${dir}/${f}`).filter((file) =>
        file.endsWith(".js")
      );
      directory
        .map((file) => require(`${dir}/${f}/${file}`))
        .forEach((cmd) => this.commands.push(new cmd(this)));
    });
  }
  ...
  loadEvents(dir) {
    readdirSync(dir)
      ...
      .forEach((ev) => {
        ...
        this[event.name === "ready" ? "once" : "on"](event.name, (...args) =>
          new event(this).execute(...args)
        );
      });
  }
